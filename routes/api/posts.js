...
router.get("/", (req, res) => {
  ...
  Post.find()
    ...
    .catch(err =>
      res.status(404).json({ postsnotfound: "Posts could not be found" })
    );
});
      ...
      .then(post => {
        Like.findOne({ post: post.id, user: req.user.id }).then(like => {
          if (!like) {
            return res
              .status(400)
              .json({ msg: "You must like before you dislike" });
          }
          like.remove().then(() => {
            post.likes_count = post.likes_count - 1;
      ...
      .then(post => {
        Comment.findById(req.params.comment_id).then(comment => {
          ...
          if (
            ...
            comment.remove().then(() => {
              Comment.find({ post: post.id }).then(comments =>
                res.json({ post: post, comments: comments })
              );
            });
          } else {
            errors.comment =
              "You do not have the authority to remove that comment";
            return res.json(errors);
          }
        });
      })
      .catch(err =>
        console.log(`Something went wrong finding the post: ${err}`)
      );
  }
);
